	...
	for _, host := range nmapResult.Host {
		...
		if host.Status.State == "up" {
			...
			for _, plugin := range plugins.GetHostPlugins() {
				wg.Add(1)
				go func(plugin plugins.Plugin, ip string) {
					defer wg.Done()
					outputFile := filepath.Join(outputDir, fmt.Sprintf("%s_host_%s.txt", target, plugin.Name()))
...
func handleOpenPort(ip, port, service, outputDir, target string) error {
	...
	for _, plugin := range plugins {
		...
		if err != nil {
			return fmt.Errorf("\033[1;31m[!] Failed to run plugin %s: %v\033[0m", plugin.Name(), err)
		}
		err = utils.WriteToFile(outputFile, output)
		if err != nil {
